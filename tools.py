...
class IDPTools:
    ...
    def categorize_document_old(self, file_paths):
        response_message = []
        binary_data_array = []
        message_list = []
        single_file = True
        ...
        if len(file_paths) == 1:
            ...
            for file_path in file_paths:
                ...
                if binary_data is None or media_type is None:
                    continue
                # Since there may be many pages for a given file, adding only the first page for classification
                ...
                message_list = [
                {
                    ...
                    "content": [
                        # {"image": {"format": media_type, "source": {"bytes": binary_data[0]}}},
                        *[{"image": {"format": media_type, "source": {"bytes": data}}} for data, media_type in binary_data_array],
                        {"text": "What types of documents are in this document?"}
                    ]
                }]

        system_message = [
            {"text": "<task>You are a document processing agent and you have perfect vision. Look through every image presneted and categorize them based on the doc_type_choices. There might be multiple types of documents on a given image. Look carefully to see what documents are there. Double check if needed. Finally produce a combined list of all the types of documents attached</task> <doc_type_choices>URLA, W2, PS, DL, or UNK</doc_type_choices> <output_format>{'docs': [{'type':[doc_type], 'file_path': [file_path]}]}</output_format>In this case PS means pay stub, DL means driver's license, and UNK means unknown. <important>Include only the answer in the <outout_format> and nothing else</important>"}
        ]

        response = self.sonnet_bedrock_utils.invoke_bedrock(message_list=message_list, system_message=system_message)
        ...
        try:
            ...
            system_message = [{
                "text": (
                    ...
                    "Finally produce a combined list of all the types of documents attached</task> "
                    f"<doc_type_choices>{', '.join(DOCUMENT_TYPES)}</doc_type_choices> "
                    "<output_format>{'docs': [{'type':[doc_type], 'file_path': [file_path]}]}</output_format>"
                    "In this case PS means pay stub, DL means driver's license, and UNK means unknown. "
                    "<important>Include only the answer in the <output_format> and nothing else</important>"
                )
            }]

            response = self.haiku_bedrock_utils.invoke_bedrock(
    ...
    def check_required_documents(self, classified_documents):
        ...
        system_message = [
            {"text": "<required_documents>URLA, Driver's License</required_documents><task>You are a mortgage agent. You main task is to verify if all the documents required (check <required_documents>)for a mortgage application are present. Double check and triple check the output if needed.</task> <output>only output a json array of the missing document type</output>"}
        ]
